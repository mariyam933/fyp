...
import { adminValidate } from "@/utils/common"
import { useState } from "react"
import toast from "react-hot-toast"

export default function AdminModal({ setRefreshUI }) {
...
//   const [meterNo, setMeterNo] = useState("")
  ...
  const createCustomer = async (e: React.FormEvent<HTMLButtonElement>) => {
    e.preventDefault()
    const errors = adminValidate(name, email, address, phone);
    ...
    <Dialog open={showModal} onOpenChange={setShowModal}>
      ...
      <DialogContent className="sm:max-w-[425px] p-6" onClick={(e) => e.stopPropagation()}>
        <DialogHeader>
          ...
          <DialogDescription>Enter the Admin details</DialogDescription>
        </DialogHeader>
        <form>
          <div className="py-4">
