...
export default function ConfirmCustomerDeleteModal({
  setRefreshUI,
  showModal,
  setShowModal,
  customerId,
  ...
  const handleDelete = async () => {
    setLoading(true)
    try {
      await axiosClient.delete(`/api/customer/${customerId}`)
  ...
  return (
    <Dialog open={showModal} onOpenChange={setShowModal}>
      <DialogContent className="p-6" onClick={(e) => e.stopPropagation()}>
        <DialogHeader>
          <DialogTitle className="text-xl">Confirm Delete Customer</DialogTitle>
          <DialogDescription>
